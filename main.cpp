...
struct Params {
    ...
    Params(const std::vector<std::vector<int>> &m1, const std::vector<std::vector<int>> &m2,
           std::vector<std::vector<int>> &resM, int blockI, int blockJ, int matrixSize, int blockSize)
...
void blocksMul(const std::vector<std::vector<int>> &m1, const std::vector<std::vector<int>> &m2,
               std::vector<std::vector<int>> &resM, int blockI, int blockJ, int matrixSize, int blockSize) {
...
void defaultMul(const std::vector<std::vector<int>> &m1, const std::vector<std::vector<int>> &m2,
                std::vector<std::vector<int>> &resM, int matrixSize, int blockSize) {
...
void *blocksMul1(void *params) {
    Params *paramsPtr = (Params *) params;
    blocksMul(paramsPtr->m1, paramsPtr->m2, paramsPtr->resM, paramsPtr->blockI, paramsPtr->blockJ,
              paramsPtr->matrixSize, paramsPtr->blockSize);
    pthread_exit(NULL);
}

void threadMul(const std::vector<std::vector<int>> &m1, const std::vector<std::vector<int>> &m2,
               std::vector<std::vector<int>> &resM, int matrixSize, int blockSize) {
    int countOfBlocks = m1.size() % blockSize == 0 ? m1.size() / blockSize : m1.size() / blockSize + 1;

    std::vector<pthread_t> threads(countOfBlocks * countOfBlocks);
    int i = 0;
    for (int blockI = 0; blockI < matrixSize; blockI += blockSize)
        for (int blockJ = 0; blockJ < matrixSize; blockJ += blockSize)
            pthread_create(&threads[i++], NULL,&blocksMul1,new Params(m1, m2, resM, blockI, blockJ, matrixSize, blockSize));
...
void test(std::vector<std::vector<int>> &m1, std::vector<std::vector<int>> &m2, std::vector<std::vector<int>> &m3,
          int matrixSize) {
...
int main() {
    ...
    getMatrix(m1, "matrix1");
    getMatrix(m2, "matrix2");
